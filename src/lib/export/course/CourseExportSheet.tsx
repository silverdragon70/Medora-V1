...
import { generateCourseClinicalPDF } from './generateCourseClinicalPDF';
import { generateCoursePlainPDF } from './generateCoursePlainPDF';
import { generateCourseWord } from './generateCourseWord';
import { generateCourseExcel, generateCourseCSV } from './generateCourseExcel';
import { writeToCache, saveToDownloads, shareFile, blobToBase64 } from '../fileHelpers';
import type { ExportFormat, ReportStyle, CourseExportData, CourseExportItem } from '../types';
...
const CourseExportSheet: React.FC<CourseExportSheetProps> = ({
  ...
  const filtered = useMemo(() => {
    if (period === 'all') return courses;
    return courses.filter(c => {
      const d = new Date(c.date);
      return !isBefore(d, startOfDay(fromDate)) && !isAfter(d, endOfDay(toDate));
    });
  }, [courses, period, fromDate, toDate]);

  const buildData = (): CourseExportData => ({
    doctorName,
    institution,
    courses: filtered,
    stats: {
      total: filtered.length,
      withCertificate: filtered.filter(c => c.hasCertificate).length,
    },
  ...
  const handleExport = async () => {
    ...
    const filename = `Courses_${format(new Date(), 'dd_MMM')}_${randomNum}`;
    try {
      let base64 = '', fname = '';
      const d = buildData();
      ...
      if (exportFormat === 'PDF') {
        const blob = reportStyle === 'clinical'
          ? await generateCourseClinicalPDF(d)
          : await generateCoursePlainPDF(d);
        base64 = await blobToBase64(blob); fname = `${filename}.pdf`;
      } else if (exportFormat === 'Excel') {
        base64 = await blobToBase64(generateCourseExcel(d)); fname = `${filename}.xlsx`;
      } else if (exportFormat === 'CSV') {
        base64 = await blobToBase64(generateCourseCSV(d)); fname = `${filename}.csv`;
      } else if (exportFormat === 'Word') {
        base64 = await blobToBase64(generateCourseWord(d)); fname = `${filename}.doc`;
      }
      ...
      <Drawer open={open} onOpenChange={onOpenChange}>
        <DrawerContent className="max-h-[85vh]">
          <DrawerHeader className="text-left pb-2">
            <DrawerTitle>Export Courses</DrawerTitle>
          </DrawerHeader>
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <button className="w-full h-10 px-3 bg-card border border-border rounded-xl text-[12px] text-left">
...
export default CourseExportSheet;
